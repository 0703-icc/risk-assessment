...
def _get_company_display_name(company_name: str) -> str:
    """获取公司显示名称（含股票代码）"""
    for code, info in _COMPANY_INFO.items():
...
class ReportGenerator:
    ...
    def generate_pdf(self, company_name: str) -> bytes:
        ...
        # 获取显示名称（含股票代码）
        display_name = _get_company_display_name(company_name)

        elements.append(Paragraph(f"{display_name} 风险评估报告", title_style))
        elements.append(Spacer(1, 20))

        report = data["report"]
        elements.append(Paragraph(f"<b>开发商名称：</b>{display_name}", normal_style))
