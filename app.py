...
from stock_lookup import resolve_company_name, BUILTIN_CREDIT_CODES, _is_credit_code
...
def parse_company_input(input_text: str) -> str:
    ...
    支持输入：股票代码、简称、全称、带后缀代码（如 01109.HK、1109.HK）、统一社会信用代码（18位）
    """
    global _DYNAMIC_CODE_CACHE
    input_text = input_text.strip()

    # 0. 处理带后缀的代码（如 01109.HK, 1109.hk, AAPL.US）
    if '.' in input_text:
        parts = input_text.split('.')
        code_part = parts[0].strip()
        suffix = parts[1].strip().upper()
        if suffix in ('HK', 'US', 'SZ', 'SH'):
            input_text = code_part
    ...
    # 2. 尝试通过股票代码接口自动识别（支持任意A股/港股/美股/信用代码）
    resolved = resolve_company_name(input_text)
    if resolved:
        company_name = resolved["name"]
        code = resolved["code"]
        # 如果是信用代码且对应有股票代码，优先缓存股票代码
        if code in BUILTIN_CREDIT_CODES and "stock" in BUILTIN_CREDIT_CODES[code]:
            _DYNAMIC_CODE_CACHE[company_name] = BUILTIN_CREDIT_CODES[code]["stock"]
        else:
            _DYNAMIC_CODE_CACHE[company_name] = code
        return company_name

    # 2.5 如果是信用代码但无法解析，显示为"未知企业（信用代码）"
    if _is_credit_code(input_text):
        _DYNAMIC_CODE_CACHE[input_text] = input_text
        return input_text
...
def get_company_code(company_name: str) -> str:
    ...
    # 3. 查信用代码映射（反向查找：名称 -> 信用代码）
    for credit_code, info in BUILTIN_CREDIT_CODES.items():
        if company_name == info["name"] or company_name in info["name"]:
            return credit_code
...
def get_company_display_name(company_name: str) -> str:
    ...
    返回格式：名字（代码），例如：招商蛇口（001979）、北京城建（911100001000238753）
    如果找不到代码，只返回公司名称
    """
    ...
    # 信用代码映射中查找
    if code in BUILTIN_CREDIT_CODES:
        info = BUILTIN_CREDIT_CODES[code]
        # 如果有对应的股票代码，优先显示股票代码
        if "stock" in info:
            return f"{info['name']}（{info['stock']}）"
...
if "show_report" in st.session_state and st.session_state.show_report:
    ...
    company_name = st.text_input(
        "开发商名称",
        placeholder="输入公司名称、简称、股票代码或统一社会信用代码",
    ...
    if matching_history:
        ...
        for idx, name in enumerate(matching_history[:5], 1):
            ...
            with col2:
                ...
                if latest_report and latest_report.get('evaluation_time'):
